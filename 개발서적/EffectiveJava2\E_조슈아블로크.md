...
### 메서드
  ...
  - 규칙 43 : null 대신 빈 배열이나 컬렉션을 반환하라
    * 빈 배열이나 컬렉션 대신 null을 반환하는 메서드는 구현하기 까다롭다.(null의 예외처리 문제)
    * null을 반환하나 빈 배열 혹은 컬렉션을 반환하나 성능상 큰 차이를 보이지 않는다.
    * 길이가 0인 배열은 변경이 불가능 하므로 아무 제약 없이 재사용할 수 있다.
    * Collections.emptySet, Collections.emptyList, Collections.emptyMap 메서드를 사용하면 된다.
    * 빈 배열을 상수로 만들고 사용하면 편하다.

  - 규칙 44 : 모든 API 요소에 문서화 주석을 달아라.
    * 사용할 수 있는 API가 되려면 문서가 있어야 한다.
    * 좋은 API 문서를 만들려면 API에 포함된 모든 클래스, 인터페이스, 생성자, 메서드, 그리고 필드 선언에 문성화 주석을 달아야 한다.
    * 직렬화가 가능한 클래스라면 직렬화 형식도 밝혀야 한다.
    * 메서드에 대한 문서화 주석은 메서드와 클라이언트 사이의 규약을 간단하게 설명해야한다.
    * 모든 문서화 주석의 첫 번째 문장은 해당 주석에 담긴 내용을 요약한 것이다.
